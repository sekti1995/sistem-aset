...
$q = mysql_query("SELECT * FROM tmp_data");
while($row = mysql_fetch_assoc($q)){
	$barr = mysql_fetch_assoc(mysql_query("SELECT id_barang, nama_barang FROM ref_barang WHERE REPLACE(nama_barang,' ','') = REPLACE('$row[b]',' ','') "));
	$id_barang = $barr['id_barang'];
	
	if($id_barang== ""){
		echo "<b style='color:red'>".$row["b"]. "</b> " .$barr["nama_barang"]. " " .$id_barang."<br>";
	} else {
		echo $row["b"]. " " .$barr["nama_barang"]. " " .$id_barang."<br>";
	}
	...
	$que = mysql_query("INSERT INTO kartu_stok 	(id_stok, uuid_skpd, id_barang, id_kelompok, id_gudang, 
									...
									VALUES	(UUID(), '$uuid_skpd', '$id_barang', '1', '24bd5f0e-a1ac-11e6-b5b7-6cae8b5fc378', 
											...
											'$datime', 0, 'IMPORT-OUT-BKD-25102018')");
	...
	$que = mysql_query("INSERT INTO kartu_stok 	(id_stok, uuid_skpd, id_barang, id_kelompok, id_gudang, 
									...
									VALUES	(UUID(), '$uuid_skpd', '$id_barang', '1', '24bd5f0e-a1ac-11e6-b5b7-6cae8b5fc378', 
											'28',
											'$uuid', '$uuid2',
											'2018-06-30', '2018', '$row[h]', '0', '$row[e]', 'i',
											'$datime', 0, 'IMPORT-OUT-BKD-25102018')");
