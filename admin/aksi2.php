...
if($module=='ganti_password'){
	...
	switch ($oper) {
        case 'add':
			$delks = mysql_query("DELETE FROM kartu_stok WHERE uuid_skpd = '$uuid_skpd' AND kode = 'ok'");
			$array_bar2 = "";
...
// AWAL AKSI UNTUK MASTER DATA //
			...
			if ( ! empty($_FILES['file_awal']['name'])) {
				...
				if (in_array($_FILES['file_awal']['type'], $csv_mimetypes) && ($_FILES['file_awal']['size'] < 600000) && ($type == 'csv')) {
					...
					if (file_exists($uploadfile)) {					
						...
						while (($data = fgetcsv($handle, 10000, $delimiter)) !== FALSE) {
							...
							if($saldo!=0){
								...
								if(mysql_num_rows($ada)>0){
									...
									$row['harga']= number_format($harga, 0, ',', '.');
						...
						$result["total"] = $r;
						$result["rows"] = $items;
						$result["total"] = number_format($ttotal, 0, ',', '.');
   ...
   if(isset($form['no_ba_pemeriksaan']))$no_ba_pemeriksaan = $form['no_ba_pemeriksaan'];
	...
	//if(isset($form['id']))$id_surat = $form['id'];
	if(isset($form['id_sub']))$id_sub = $form['id_sub'];
	if(isset($form['ta']))$ta = $form['ta'];
	if(isset($form['no_surat']))$no_surat = $form['no_surat'];
	if(isset($form['tgl_surat']))$tgl_surat = balikTanggal($form['tgl_surat']);
