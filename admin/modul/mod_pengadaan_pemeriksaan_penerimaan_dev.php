	...
	<table cellpadding="2" border=0>
		...
		<tr>
			...
			<td>
				...
				Penyedia <input class="easyui-textbox" type="text" name="nama_penyedia" id="nama_penyedia" data-options="required:true"  style="width:150px;" ></input>
				Kontrak <input class="easyui-textbox" type="text" name="no_kontrak" id="no_kontrak" data-options="required:true"  style="width:100px;" ></input>
			</td>
		</tr>
		<tr>
			...
			$.get( "./model/get_lock_date.php", function( data ) {
				...
				$('#tgl_pembayaran').datebox().datebox('calendar').calendar({
					validator: function(date){
						...
						var d2 = new Date(thn2, bln2, tgl2+1);
						var d3 = new Date(2019, 12, 31);
						return d2<=date && date<=d3;
					}
				});
...
function newPengadaan(){
	...
	$('#basket').datagrid('loadData', {"total":0,"rows":[],"footer":[]});
	urlu = './aksi.php?module=pengadaan_baru&oper=add'; 
...
function editPengadaan(){
	...
	if(row.stat == "x"){
		...
		if (row){
			...
			editIndex = undefined; 
			aksi = "edit";
			urlu = './aksi.php?module=pengadaan_baru&oper=edit&id_ubah='+row.id;
...
function destroyPengadaan(){
	...
	if (rw1){
		...
		if (r){
			$.ajax({
				type: "POST",
				url: './aksi.php?module=pengadaan_baru&oper=del',
		...
		function onBeginEdit(rowIndex){
			...
			var harga = $(editors[8].target);
			var hargasat = $(editors[7].target);
			var satuan = $(editors[6].target);
			var fbar = $(editors[10].target);
			var fsat = $(editors[11].target);
			var fkel = $(editors[12].target);
			var frek = $(editors[14].target);
			var fsubrek = $(editors[15].target);
