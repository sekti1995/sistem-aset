...
<input type="text" name="tgl_max" id="tgl_max" hidden /></td>
...
function cekBarang(){
	if($('#tgl_surat').datebox('getValue')==''){
		...
		if(d1 < d2){
			$.messager.alert('Peringatan', 'Tanggal SPPB tidak bisa mundur lebih dari tanggal SPPB yang sudah ada ! - ' + tgl_akhir);
		}else{
...
function savePerintah(){
	if($('#tgl_surat').datebox('getValue')==''){
		$.messager.alert('Peringatan', 'Tanggal Surat belum diisi !');
	}else{
		var d1 = new Date(myparser($('#tgl_surat').datebox('getValue')));
		var d2 = new Date(myparser($('#tgl_max').val()));
		if(d1 < d2){
			$.messager.alert('Peringatan', 'Tanggal Surat tidak bisa mundur lebih dari tanggal Surat yang sudah ada ! ');
			return false;
		}
	...
	if (rw1){
		if(rw1.status==1){
			...
			if (r){
				$.ajax({
					...
					data: { id_hapus: rw1.idsp, tgl_surat: rw1.tgl_surat },
					success: function(data){
						console.log(data);
						var data = eval('('+data+')');
						if (data.success==false){
        ...
        function append(){
			if($('#tgl_surat').datebox('getValue')==""){ 
				...
				var d2 = new Date(myparser(tgl_akhir));
				if(d1 < d2){
					$.messager.alert('Peringatan', 'Tanggal SPPB tidak bisa mundur lebih dari tanggal SPPB yang sudah ada ! '+tgl_sp_akhir);
					return;
				}
				$('#tgl_surat').datebox('readonly', true);
				if (endEditing()){
