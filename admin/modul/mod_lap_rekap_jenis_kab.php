 ...
 <table id="dgfull"class="easyui-datagrid"
	...
	toolbar="#toolbar" title="Laporan Rekapitulasi Jenis Barang Kabupaten"
...
<div id="dls" class="easyui-dialog" <?php if($_SESSION["jenis"] == "UPB" || $_SESSION["jenis"] == ''){ ?> 
		style="width:350px;height:250px;padding:10px 20px" 
	<?php } else { ?> 
		style="width:450px;height:350px;padding:10px 20px" 
	<?php }?>
closed="true" buttons="#dls-buttons" title="Pencarian Data Rekapitulasi Jenis Barang Kabupaten">
<div class="ftitle">Pencarian Data Rekapitulasi Jenis Barang Kabupaten</div>
...
<?php // if($peran==md5('1') or $peran==md5('7')){ ?>
	...
	<div class="fitem" id='filt_cetak'>
		...
		<input class="easyui-combobox" style="width:180px;" id="id_akses" name="id_akses" />
		<script>
		var id_akses;
		var opd;
		$('#id_akses').combobox({
	...
	<div class="fitem" id='filt_opd'>
		<label>OPD</label>: 
		<input class="easyui-combobox" style="width:250px;" id="id_unit" name="id_unit" />
		<script>
		$('#id_unit').combobox({
...
$(function(){
	// console.log("<?php echo $_SESSION['jenis']; ?>");
...
function CetakJenis2(){
	...
	if(tglawal == '' || tglakhir == ''){
		alert("Silahkan isi range tanggal di menu pencarian terlebih dahulu !");
		return;
	}
