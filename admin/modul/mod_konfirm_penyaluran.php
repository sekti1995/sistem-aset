...
<thead>
<tr>
<th field="nm_sub" width="100" align="left" halign="center">Nama Sub2/Unit Kerja</th>
...
<th field="tt" width="60" align="left" halign="center">JML Barang</th>
<th field="create_date" width="120" align="left" halign="center">Tanggal Entri</th>
...
<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-print" plain="true" onclick="cetak()">Cetak Penyaluran Barang</a>
<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-print" plain="true" onclick="cetakBA()">Cetak BA Penyaluran Barang</a>
...
function cetak(){
	...
	if (row){
		...
		$.post( "./print/lap_penyaluran.php", { id: row.id, pengirim:row.nm_skpd, penerima:row.nm_sub, tgl_terima:row.tgl_terima, id_pengirim:row.id_sub, id_penerima:row.id_subt, id_gudang:row.id_gudang })
		.done(function( data ) {
			console.log(data)
...
function cetakBA(){
	...
	if (row){
		$.loader.open($dataLoader);
		$.post( "./print/ba_penyaluran.php", { id: row.id, pengirim:row.nm_skpd, penerima:row.nm_sub, tgl_terima:row.tgl_terima, id_pengirim:row.id_sub, id_penerima:row.id_subt, id_gudang:row.id_gudang, nomor:row.nomor , ta:row.ta })
		.done(function( data ) {
			console.log(data)
			if(data.success==false) alert(data.pesan);
			window.location.href = data.url;
	...
	}else $.messager.alert('Peringatan','Pilih Data Penyaluran Barang yang akan dicetak!');	
}
