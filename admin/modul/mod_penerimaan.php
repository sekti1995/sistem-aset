 ...
 <table id="dgfull"class="easyui-datagrid"
	url="./model/pengadaan.php?sp=2" fit="true"
	toolbar="#toolbar" pagination="true" title="Input Penerimaan Barang Persediaan"
...
<div id="toolbar">
<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editPenerimaan()">Penerimaan</a>
<!--
<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="destroyPenerimaan()">Batal Penerimaan</a>
-->
<div style="float: right; margin-right: 5px;">
	<a href="#" class="easyui-linkbutton" plain="true" iconCls="icon-search" onclick="viewSearch()">Pencarian</a>
</div>
</div>
<div id="dlg" class="easyui-dialog" style="width:400px;height:250px;padding:5px 10px"
...
<td colspan="2">: <input class="easyui-textbox" type="text" name="no_ba_penerimaan" id="no_ba_penerimaan" data-options="required:true"  style="width:100px;" ></input></td>
</tr>
<tr>
<td>Tgl Penerimaan</td>
<td colspan="2">: <input class="easyui-datebox" type="text" name="tgl_penerimaan" id="tgl_penerimaan" data-options="formatter:myformatter,parser:myparser,required:true" validType="validDate" style="width:100px;"  readonly="readonly"></td>
</tr>
<tr>
<td>No Dokumen</td>
<td colspan="2">: <input class="easyui-textbox" type="text" name="no_dok_penerimaan" id="no_ba_penerimaan" data-options="required:true"  style="width:100px;" ></input></td>
</tr>
<tr>
<td>Tgl Dokumen</td>
<td colspan="2">: <input class="easyui-datebox" type="text" name="tgl_dok_penerimaan" id="tgl_penerimaan" data-options="formatter:myformatter,parser:myparser,required:true" validType="validDate" style="width:100px;"  readonly="readonly"></td>
</tr>
<tr>
<td>Gudang/Tempat</td>
<td colspan="2">: <input class="easyui-combobox" style="width:150px;" id="id_gud" name="id_gud" required="true"/>
<script>
$('#id_gud').combobox({
    valueField:'id_gud',
    textField:'nama_gud',
...
<div id="dlg-buttons">
<a href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="savePenerimaan()" style="width:90px">Save</a>
...
function viewSearch(){
	$('#dls').dialog('open').dialog('setTitle','Pencarian Data Penerimaan');
	$('#fms').form('clear');
}	
function doSearch(){
...
function editPenerimaan(){
	var row = $('#dgfull').datagrid('getSelected');
	if (row){
		$('#dlg').dialog('open').dialog('setTitle','Penerimaan');
		$('#id_gud').combobox('reload', './model/cb_gudang.php?id='+row.id_sub);
		$('#fm').form('clear');
		$('#fm').form('load',row);
		urlu = './aksi.php?module=penerimaan&oper=edit&id_ubah='+row.id;
	}else $.messager.alert('Peringatan','Pilih Data Pengadaan yang akan diterima !');	
}

function savePenerimaan(){	
	if($('#fm').form('validate')==false){
		$.messager.show({ title: 'Error', msg: 'Data Penerimaan belum diisi' });
...
function destroyPenerimaan(){
	var rw1 = $('#dgfull').datagrid('getSelected');
	if (rw1){
		$.messager.confirm('Peringatan','Apakah Anda yakin akan menghapus Penerimaan ini?',function(r){
		if (r){
				$.ajax({
					type: "POST",
					url: './aksi.php?module=penerimaan&oper=del',
	...
	}else $.messager.alert('Peringatan','Pilih Penerimaan yang akan dihapus dahulu !');	
}
