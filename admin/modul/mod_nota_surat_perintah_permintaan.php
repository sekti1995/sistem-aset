...
$('#id_sub').combobox({
	...
	onSelect: function(rec){
		//console.log(rec.tgl_max)
		//$('#tgl_max').val(rec.tgl_max);
		id_sub = rec.id;
		idskpd = rec.idskpd;
		nmskpd = rec.nmskpd;
...
<tr>
	...
	<td width="100"> </td>
	<td>Tanggal SPPB</td>
	<td>: <input class="easyui-datebox" type="text" name="tgl_surat" id="tgl_surat" data-options="formatter:myformatter,parser:myparser,required:true" style="width:100px;" validType="validDate">
   ...
   <table id="basket" fitColumns="true" rownumbers="true"  style="width1:150px;height:270px;" toolbar="#tb" url="./model/verifikasi_barang.php" title="Data Barang">
	   <thead>
		   <tr>
			   ...
			   <th field="harga" width=110 align="right" halign="center" data-options="editor: {type:'textbox', options:{ readonly:'true'}}" hidden>Harga</th>
...
$(function(){	
	...
	$('#tanggal').datebox({
		onSelect: function(date){ 
			...
			if(parseInt(ta) != parseInt(tap)){
				...
				$('#tanggal_spb').datebox('clear');
				$('#tgl_surat').datebox('clear');
	...
	$('#tgl_surat').datebox({
		onSelect: function(date){ 
			...
			var tgl = $('#tgl_surat').datebox('getValue');
			var uuid_skpd = $('#id_sub').combobox('getValue');
			var ta = $("#ta").combobox('getValue');
			var tap = tgl.substring(6,10);
			if(parseInt(ta) != parseInt(tap)){
	...
	$('#tanggal_spb').datebox({
		onSelect: function(date){ 
			...
			var tgl = $('#tanggal_spb').datebox('getValue');
			var uuid_skpd = $('#id_sub').combobox('getValue');
			var ta = $("#ta").combobox('getValue');
			var tap = tgl.substring(6,10);
			if(parseInt(ta) != parseInt(tap)){
...
/* function formatStok1(val,row){ 
 	if(val!=undefined){
		var cek1 = row.jmlkeluar;
		var cek2 = cek1.replace(".","");
		var tot1 = parseInt(cek2) - parseInt(val) ;
		var tot2 = parseInt(val) + parseInt(tot1);
		//alert(cek1);
		//alert(val);
		//alert(tot2);
		if (parseFloat(cek1) > parseFloat(val)){
			return '<span style="color:red;">('+tot2+')</span>';
		} else {
			return val;
		}
	}
