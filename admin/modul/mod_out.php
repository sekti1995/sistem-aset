...
<div id="toolbar">
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="newNota()">Pengeluaran Baru</a>
	<!--
	...
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-print" plain="true" onclick="cetakPerintah()">Cetak Surat Perintah Keluar</a>
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-print" plain="true" onclick="cetakBA()">Cetak BA Pengeluaran</a>
	<div style="float: right; margin-right: 5px;">
		<a href="#" class="easyui-linkbutton" plain="true" iconCls="icon-search" onclick="viewSearch()">Pencarian</a>
	</div>
</div>
<div id="dlg" class="easyui-dialog" style="width:800px;height:560px;padding:5px 10px"
...
<tr>
			...
			<td colspan='5'> :
				<input class="easyui-combobox" style="width:40%;" id="id_sumber" name="id_sumber" required="true"/>
				<script>
				$('#id_sumber').combobox({
...
<div style="background:#fff">
   <table id="basket" fitColumns="true" rownumbers="true"  style="width1:150px;height:270px;" toolbar="#tb" url="./model/verifikasi_barang.php" title="Data Barang">
	   <thead>
		   <tr>
			   <th data-options="field:'nama_bar',width:300, align:'left', halign:'center',
                        formatter:function(value,row){
                            return row.nama_bar;
                        }">Nama Barang</th>
			   <th field="nama_sat" width=50 align="center" data-options="editor: {type:'textbox', options:{ readonly:'true'}}">Satuan</th>
			   <th field="jmlminta" width=100 align="center" data-options="editor: {type:'textbox' }, formatter:formatStok" hidden>Jml Minta</th>
				<!--
			   <th data-options="field:'nama_gud',width:160, align:'left', halign:'center',
                        formatter:function(value,row){
                            return row.nama_gud;
                        }" hidden>Dari Tempat</th>
			   <th data-options="field:'nama_sumber',width:160, align:'left', halign:'center',
                        formatter:function(value,row){
                            return row.nama_sumber;
                        }">Sumber Dana</th>
				-->
			   <th field="jmlstok" width=130 align="center" halign="center" data-options="editor: {type:'textbox', options:{ readonly:'true'}}">Jml Stok</th>
			   ...
			   <th field="id_sum" width=80 align="left" halign="center" editor="textbox"   >id_sum</th>
			   <th field="id_barang_selected" width=80 align="left" halign="center" editor="textbox" hidden >id_barang_selected</th>
		</tr>
		</thead>
	</table>	
</div>
...
var urlu; var id_bar; var id_sat; var id_gud; var id_kel; var datser; var id_sub; var idskpd; var nmskpd; var id_surat; var idgudang; var idsumberdana;
...
function newNota(){
	...
	$('#basket').datagrid('loadData', {"total":0,"rows":[],"footer":[]});
	urlu = './keluar.php?module=nota_minta_baru&oper=add';
	id_sub = $('#id_sub').combobox('getValue');
	editIndex = undefined;
	//$("input[name='jenis'][value='sendiri']").prop("checked", true);
	$('#txtuntuk').textbox('readonly',false);
	$('#lblurut').html('');
	//$('#id_sumber').combobox('reload', './model/cb_sumber_dana.php?cek&semua=no&id='+id_sub);
...
function editNota(){
	...
	if (row){
		if(row.status==1){
			...
			id_sub = row.id_sub;
			urlu = './keluar.php?module=nota_minta_baru&oper=edit&id_ubah='+row.id;
			editIndex = undefined;
	...
	if (rw1){
		if(rw1.status==1){
			...
			if (r){
				$.ajax({
					type: "POST",
					url: './keluar.php?module=nota_minta_baru&oper=del',
		...
		function onBeginEdit(rowIndex){
			...
			//var gudang = $(editors[3].target);
			//var sumber = $(editors[4].target);
			...
			var fsumberdana = $(editors[10].target);
			var fidbarang = $(editors[11].target);
			//var fgud = $(editors[9].target);
			//var fsum = $(editors[10].target);
			
				//var valgudang = $(editors[3].target).val();
				//var valsumber = $(editors[4].target).val();
			...
			barang.combobox({
				onSelect: function(rec){
					harga.textbox('setValue', rec.harga_format);
					var id_unit2 = $('#id_sub').combobox('getValue');
					//console.log(id_unit2);
					$.loader.open($dataLoader);
					$.post( "./model/cek_stok.php", { id_bar: rec.id, id_sub: id_unit2, id_surat: id_surat, idgudang: 0, id_sum: $("#id_sumber").combobox('getValue') })
					.done(function( data ) { 
						console.log(data)
					...
					fidbarang.textbox('setValue', rec.id);
					fsumberdana.textbox('setValue', $("#id_sumber").combobox('getValue'));
				}
			}).combobox('setValue',a);
			...
			sumber.combobox({
				onSelect: function(rec){
					...
					fgud.textbox('setValue', rec.id_gud);
					$.loader.open($dataLoader);
					$.post( "./model/cek_stok.php", { id_bar: barang.combobox('getValue'), id_sub: id_unit2, id_surat: id_surat, id_sum: rec.id })
					.done(function( data ) {
		...
		function onBeforeEdit(row){
			...
			combar.editor = {
				...
				options:{
					...
					textField:'nama_bar',
					method:'get',
					url:'./model/cb_barang_ada.php?jns_brg='+jns_brg+'&id='+$('#id_sub').combobox('getValue')+'&sd='+$('#id_sumber').combobox('getValue'),
					required:true,
					filter: function(q, row){
						var opts = $(this).combobox('options');
						return row[opts.textField].toUpperCase().indexOf(q.toUpperCase()) >= 0;
			...
			comgud.editor = {
				...
				options:{
					...
					textField:'nama_gud',
					method:'get',
					url:'./model/cb_gudang.php?cek&id='+$('#id_sub').combobox('getValue'),
					required:true,
					filter: function(q, row){
						var opts = $(this).combobox('options');
						return row[opts.textField].toUpperCase().indexOf(q.toUpperCase()) >= 0;
...
function formatStok(val,row){ 
 	if(val!=undefined){
		var cek1 = row.jmlstok;
		var cek2 = cek1.replace(".","");
		if (parseFloat(val) > parseFloat(cek2)){
...
function cetakBA(){
	...
	if (rw1){
		$.loader.open($dataLoader);
		$.post( "./print/ba_keluar_barang.php", { id : rw1.id_ba })
		.done(function( data ) {
			window.location.href = data.url;
			$.loader.close($dataLoader);
		});
...
function cetakSuratMinta(){
	...
	if (rw1){
		$.loader.open($dataLoader);
		$.post( "./print/surat_permintaan.php", { id : rw1.id_sm })
		.done(function( data ) {
			window.location.href = data.url;
			$.loader.close($dataLoader);
		});
