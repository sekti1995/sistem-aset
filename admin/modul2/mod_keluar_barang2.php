...
function saveKeluar(){
	...
	if($('#jenis_out').val()!='r'){
		$.each(basketSP, function(j,isi){
			var jmlSP = parseFloat(isi['jml_barang'] );
			var jmlkel = 0;
			$.each(basket.rows, function(i,lab){
				if(lab['id_bar']==isi['id_barang']){
					jmlkel += parseFloat(lab['jumlah'] );
				}	
			});
...
function destroyKeluar(){
	...
	if (rw1){
						...
						if(isi['id_barang']==barang){
							...
							$.each(basket.rows, function(i,lab){
								if(lab['id_bar']==barang && i!=editIndex){
									jmlbas += parseFloat(lab['jumlah'].replace(/\D/g, ""));
								}	
							});
							jmlbas+=parseFloat(jumlah.replace(/\D/g, ""));
							
							if(jmlbas>parseFloat(isi['jml_barang'].replace(/\D/g, ""))) sesuai = "tidak";
						}
					});
					if(sesuai=="tidak"){
		...
		function onBeforeEdit(row){
			...
			//var tglakhir = $(this).datagrid('getColumnOption','tgl_akhir');
