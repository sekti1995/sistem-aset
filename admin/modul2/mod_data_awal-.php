	...
	if(dg.total==0){
        ...
        function endEditing(){
            ...
            if ($('#dg').datagrid('validateRow', editIndex)){
				...
				var harga = $(ed[4].target).val();
				
				var sama = "";
				var basket = $('#dg').datagrid('getData');
				$.each(basket.rows, function(i,lab){
					if(lab['id_bar']==barang && harga==lab['harga'] && i!=editIndex) sama = 'ya';
				});
				
				if(validasiCombo2(ed)==false) return false;	
				if(sama=='ya'){
					$.messager.show({ title: 'Error', msg: "Barang dengan harga ini Sudah ada dalam daftar!" }); 
					return false;
				}else{
			...
			barang.textbox('textbox').bind('keyup',function(e){
				...
				var nmbrg = $this.val();
					...
					if(nmbrg.length >= 3){
						...
						barang.combobox('reload', './model/cb_barang.php?search='+nmbrg+'&kel='+id_kel+'&idsub='+id_sub);
		...
		function onBeforeEdit(row){
			...
			combar.editor = {
				...
				options:{
					...
					textField:'nama_bar',
					method:'get',
					url:'./model/cb_barang.php?kel='+id_kel+'&idsub='+id_sub,
					required:true,
					filter: function(q, row){
						var opts = $(this).combobox('options');
						return row[opts.textField].toUpperCase().indexOf(q.toUpperCase()) >= 0;
