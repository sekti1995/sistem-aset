...
// AWAL AKSI UNTUK MASTER DATA //
   ...
   if(isset($_REQUEST['id_jabatan']))	$id_jabatan = $_REQUEST['id_jabatan']; else $id_jabatan = 0;
   if(isset($_REQUEST['id_golongan']))	$id_golongan = $_REQUEST['id_golongan']; else $id_golongan = 0;
   if(isset($_REQUEST['ta']))$ta = $_REQUEST['ta'];
   ...
   if(isset($_REQUEST['state']))$state = 0; else $state = 1;
	...
	switch ($oper) {
        case 'up':
			...
			if ( ! empty($_FILES['file_awal']['name'])) {
				...
				if (in_array($_FILES['file_awal']['type'], $csv_mimetypes) && ($_FILES['file_awal']['size'] < 600000) && ($type == 'csv')) {
					...
					if (file_exists($uploadfile)) {					
						...
						while (($data = fgetcsv($handle, 10000, $delimiter)) !== FALSE) {
							...
							if($saldo!=0){
								$ada = mysql_query("SELECT id_barang FROM ref_barang WHERE id_barang = '$data[0]'
												AND nama_barang = '$data[2]' AND soft_delete = 0");
...
//if(isset($_REQUEST['id_sub']))$uid_skpd = $_REQUEST['id_sub'];
   ...
   if(isset($_REQUEST['basket']['rows']))$basket = $_REQUEST['basket']['rows']; else $basket = array();
   if(isset($_REQUEST['basrinci']))$basrinci = $_REQUEST['basrinci']; else $basrinci = array();
   ...
   if(isset($form['tgl_penerimaan']))$tgl_penerimaan = balikTanggal($form['tgl_penerimaan']);
	...
	switch ($oper) {
        ...
        case 'edit': 
			...
			$datacek = array();
			$edit = array(); $add = array(); $del = array(); $cek = ""; $addrinci = array(); $delrinci = array(); $delrinciall = array();
   ...
   if(isset($form['no_ba_pemeriksaan']))$no_ba_pemeriksaan = $form['no_ba_pemeriksaan'];
				...
				foreach($basket AS $val){
					...
					mysql_query("INSERT INTO masuk_detail ( id_masuk_detail, id_masuk, uuid_skpd,
							...
							mysql_query("INSERT INTO kartu_stok (id_stok, uuid_skpd, id_barang, id_kelompok, id_gudang, 
																...
																tgl_transaksi, ta, jml_in, jml_out, harga, kode,
																create_date, soft_delete, creator_id)
														VALUES	(UUID(), '$uid_skpd', '$val[id_bar]', '$val[id_kel]','$id_gudang', 
	...
	switch ($oper) {
        case 'edit':
			if($sp=='1') $sta = "status_proses = '2',"; else $sta = "";
	...
	switch ($oper) {
		...
		case 'edit':
			...
			$edit = array(); $add = array(); $del = array(); $array_bar = ""; $array_jml = "";
			while($da = mysql_fetch_assoc($datser)){
				$cek = "";
				foreach ($basket as $key => $val){
					if(isset($val['id'])){ //data lama 
	...
	if(isset($form['jenis_out'])) $jenis_out = $form['jenis_out']; else $jenis_out = "";
	if(isset($form['tgl_minta'])) $tgl_minta = $form['tgl_minta'];
	if(isset($form['no_reklas']))$no_reklas = $form['no_reklas']; else $no_reklas = "";
	if(isset($form['tgl_reklas']))$tgl_reklas = balikTanggal($form['tgl_reklas']); else $tgl_reklas = "";
	...
	if(isset($form['nomor']))$nomor = $form['nomor'];
	if(isset($form['tanggal']))$tanggal = balikTanggal($form['tanggal']);
	if(isset($form['id_pengurus']))$id_pengurus = $form['id_pengurus']; else $id_pengurus = "";
	if(isset($form['id_pengguna']))$id_pengguna = $form['id_pengguna']; else $id_pengguna = "";
				...
				while($kd = mysql_fetch_assoc($keldel)){
					...
					mysql_query("INSERT INTO kartu_stok (id_stok, uuid_skpd, id_barang, id_kelompok, id_gudang, 
												...
												VALUES	(UUID(), '$id_sub', '$kd[id_barang]', '$kd[id_kelompok]', '$id_gudang', 
														...
														'$tgl_terima', '$kd[ta]', '$kd[jml_barang]', 0, '$kd[harga_barang]', 'r',
														'$datime', 0, '$pengguna')");
	...
	$q = mysql_query("UPDATE kartu_stok SET jml_in = '$jml_in', jml_out = '$jml_out', harga = '$harga',id_sumber_dana = '$id_sumber_dana',ta = '$ta_stok', tgl_transaksi = '$tgl_transaksi', soft_delete = '$soft_delete' WHERE id_stok = '$id_stok' ");
	if($kode_stok == 'ok'){		
				...
				foreach ($basket as $key => $val){
					if(isset($val['id'])){ //data lama 
						if($val['id']==$da['id']){ //data lama masih ada
							...
							if($da['id_bar']!=$val['id_bar']){ $isi .= "id_barang = '$val[id_bar]', "; $isi2 .= $isi; }
							if($da['jumlah']!=$jumlah){
