...
foreach (glob("../xls/Laporan Mutasi Tahunan*.*") as $filename) {
    unlink($filename);
}
...
$objPHPExcel->getActiveSheet()
			...
			->mergeCells('D6:D7')
            ->setCellValue('E6', "MUTASI Tahun $ta")
			->mergeCells('E6:F6')
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Laporan Mutasi Tahunan');
...
header('Content-Disposition: attachment;filename="Laporan Mutasi Tahunan.xlsx"');
...
$objWriter->save('../xls/Laporan Mutasi Tahunan.xlsx');
$response = array( 'success' => true, 'url' => './xls/Laporan Mutasi Tahunan.xlsx' );
header('Content-type: application/json');
