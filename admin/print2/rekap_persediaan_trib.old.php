...
$objPHPExcel->getActiveSheet()
			...
			->mergeCells('E6:E7')
			->setCellValue('F6', "SISA TRIB LALU")
			->mergeCells('F6:G6')
			->setCellValue('F7', "JML")
			->setCellValue('G7', "JML HARGA")
			->setCellValue('H6', "PENGADAAN TRIB INI")
			->mergeCells('H6:I6')
			->setCellValue('H7', "JML")
			->setCellValue('I7', "JML HARGA")
			->setCellValue('J6', "JML TRIB INI")
			->mergeCells('J6:K6')
			->setCellValue('J7', "JML")
			->setCellValue('K7', "JML HARGA")
			->setCellValue('L6', "PEMAKAIAN TRIB INI")
			->mergeCells('L6:M6')
			->setCellValue('L7', "JML")
			->setCellValue('M7', "JML HARGA")
			->setCellValue('N6', "SISA TRIB INI")
			->mergeCells('N6:O6')
			->setCellValue('N7', "JML")
...
$row = 9; $no = 1; $ttotal = 0; $kode = ""; $jml = count($basket);
		$totalJenis = 0;
		$total1 = 0;
		$total2 = 0;
		$total3 = 0;
...
foreach($basket AS $d){
	...
	$jml_lalu =  str_replace('.','',$d['jml_lalu']);
	$tot_lalu =  str_replace('.','',$d['tot_lalu']);
	$jml_in =  str_replace('.','',$d['jml_in']);
	$tot_in =  str_replace('.','',$d['tot_in']);
	$jml_ini =  str_replace('.','',$d['jml_ini']);
	$tot_ini =  str_replace('.','',$d['tot_ini']);
	$jml_out =  str_replace('.','',$d['jml_out']);
	$tot_out =  str_replace('.','',$d['tot_out']);
	$jumlah =  str_replace('.','',$d['jumlah']);
	...
	if($kode!=$d['kode_bar']){
		if($no!=1){
			$objPHPExcel->getActiveSheet()
				...
				->setCellValue('F'.$row, "$total1")
				->setCellValue('G'.$row, "$total2")
				->setCellValue('H'.$row, "$total3")
				->setCellValue('I'.$row, "$total4")
				->setCellValue('J'.$row, "$total5")
				->setCellValue('K'.$row, "$total6")
				->setCellValue('L'.$row, "$total7")
				->setCellValue('M'.$row, "$total8")
				->setCellValue('N'.$row, "$total9")
				->setCellValue('O'.$row, "$totalJenis");
			$row++;
		}
		$totalJenis = 0;
		$total1 = 0;
...
$objPHPExcel->getActiveSheet()->getStyle("E9:O$row")->getNumberFormat()->setFormatCode("#,##0.000000");			
