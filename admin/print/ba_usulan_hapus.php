...
$usulan = mysql_query("SELECT h.ta, no_ba_usulan, tgl_ba_usulan, nm_sub2_unit, no_ba_penunjukan, thn_ba_penunjukan,
				nama_alasan_penghapusan, nama_aksi_penghapusan,
				...
				LEFT JOIN ref_alasan_penghapusan a ON h.id_alasan_penghapusan = a.id_alasan_penghapusan
				LEFT JOIN ref_aksi_penghapusan k ON h.id_aksi_penghapusan = k.id_aksi_penghapusan
				WHERE id_usul_hapus = '$id'");
$detail = mysql_query("SELECT nama_barang, jml_barang, harga_barang, baik, ringan, berat, kadaluarsa 
						FROM usul_hapus_detail h
						LEFT JOIN ref_barang b ON b.id_barang = h.id_barang
						WHERE id_usul_hapus = '$id' AND h.soft_delete = 0");				
$data = mysql_fetch_assoc($usulan);
$count = mysql_num_rows($detail);	
$tgl_ba = strtotime($data['tgl_ba_usulan']);
$tgl = date('d', $tgl_ba);
$hari = getHari(date('N', $tgl_ba));
$bulan = getBulan(date('m', $tgl_ba));
$tglini = tgl_indo($data['tgl_ba_usulan']);

// Template processor instance creation
$templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('../doc/Template BA Usulan Penghapusan.docx');

// Variables on different parts of document
//print_r($templateProcessor->getVariables());

$templateProcessor->setValue('Tahun', htmlspecialchars($data['ta']));
$templateProcessor->setValue('NomorBA', htmlspecialchars($data['no_ba_usulan']));
...
$templateProcessor->setValue('Alasan', htmlspecialchars($data['nama_alasan_penghapusan']));
$templateProcessor->setValue('TindakLanjut', htmlspecialchars($data['nama_aksi_penghapusan']));
...
$templateProcessor->setValue('NamaAnggota', htmlspecialchars($data['id_pejabat_anggota1']));
...
$templateProcessor->setValue('JabDinAnggota', htmlspecialchars($data['jabatan_anggota1']));
...
while($d = mysql_fetch_assoc($detail)){
	...
	$templateProcessor->setValue('Ket#'.$i, htmlspecialchars(''));
	$i++;
}

//echo date('H:i:s'), ' Saving the result document...';
$templateProcessor->saveAs('../doc/BA Usulan Penghapusan.docx');

$response = array( 'success' => true, 'url' => './doc/BA Usulan Penghapusan.docx' );
