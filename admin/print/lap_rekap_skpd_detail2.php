...
foreach (glob("../xls/Laporan Rekapitulasi per UPTD*.*") as $filename) {
    unlink($filename);
}
...
$kd = explode('.', $kode);
if($kd[3]==1){ $in = "(8,10)"; $kep = 8; }
else { $in = "(9,11)"; $kep = 9; }
$txtPengurus = "Pengurus Barang";			
$pejabat = mysql_query("SELECT nama_pejabat, nip, id_jabatan FROM pejabat d WHERE id_jabatan IN $in AND uuid_skpd = '$id'");
...
$clause = "SELECT uuid_sub2_unit AS id, nm_sub2_unit AS nama_sub, 
			...
			FROM ref_sub2_unit 
			WHERE CONCAT_WS('.',kd_urusan,kd_bidang, kd_unit, kd_sub) = '$kode'
...
$objPHPExcel->getActiveSheet()
            ->setCellValue('A1', 'LAPORAN REKAPITULASI PERSEDIAAN PER UPTD')
			->mergeCells('A1:D1');
$objPHPExcel->getActiveSheet()
...
foreach($items AS $d){
	...
	$objPHPExcel->getActiveSheet()
            ...
            ->setCellValue('B'.$row, "$d[kode_sub]")
            ->setCellValue('C'.$row, "$d[nama_sub]")
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Laporan Rekapitulasi per UPTD');
...
header('Content-Disposition: attachment;filename="Laporan Rekapitulasi per UPTD.xlsx"');
...
$objWriter->save('../xls/Laporan Rekapitulasi per UPTD.xlsx');
$response = array( 'success' => true, 'url' => './xls/Laporan Rekapitulasi per UPTD.xlsx' );
header('Content-type: application/json');
