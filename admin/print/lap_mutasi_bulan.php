...
foreach (glob("../xls/Laporan Mutasi Bulanan*.*") as $filename) {
    unlink($filename);
}
...
$objPHPExcel->getActiveSheet()
            ->setCellValue('A1', 'LAPORAN BULANAN MUTASI BARANG PERSEDIAAN')
			->mergeCells('A1:J1');
$objPHPExcel->getActiveSheet()
...
$objPHPExcel->getActiveSheet()
			...
			->mergeCells('D6:D7')
            ->setCellValue('E6', "MUTASI Bulan $bulan")
			->mergeCells('E6:F6')
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Laporan Mutasi Bulanan');
...
header('Content-Disposition: attachment;filename="Laporan Mutasi Bulanan.xlsx"');
...
$objWriter->save('../xls/Laporan Mutasi Bulanan.xlsx');
$response = array( 'success' => true, 'url' => './xls/Laporan Mutasi Bulanan.xlsx' );
header('Content-type: application/json');
