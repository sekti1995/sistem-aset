<?php
	...
	if($jenis=='kb'){
		...
		$total = number_format(($jumlah * $harga), 15, ',', '.');
		if($jumlah>$saldo) $response = array('hasil' => false, 'jumlah' => number_format($saldo, 15, ',', '.'), 'saldo'=>number_format($r1['saldo'], 15, ',', '.'), 'pesan'=>number_format($r1['pesan'], 15, ',', '.'));
		else $response = array('hasil' => true, 'jumlah' => number_format($saldo, 15, ',', '.'), 'harga'=> number_format($harga, 15, ',', '.'), 'total' => $total);
	}else{
		//$saldo = $r1['saldo'];
		$harga = is_null($r2['harga']) ? 0 : $r2['harga'];
		$total = number_format(($saldo * $harga), 15, ',', '.');
		$response = array( 'saldo' => number_format($saldo, 15, ',', '.'), 'harga'=>number_format($harga, 15, ',', '.'), 'total'=>$total);
	}
