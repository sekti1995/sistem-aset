<?php
	...
	if($tglawal!="" AND $tglakhir!=""){
		$c = " AND DATE_FORMAT(tgl_terima, '%Y-%m-%d')  BETWEEN CAST('$tglawal' AS DATE) AND CAST('$tglakhir' AS DATE)"; 
		$cm = " AND DATE_FORMAT(tgl_transaksi, '%Y-%m-%d') < '$tglawal'"; 
		$c1 = " AND DATE_FORMAT(tgl_transaksi, '%Y-%m-%d') < '$tglawal'"; 
		$c2 = " AND DATE_FORMAT(tgl_transaksi, '%Y-%m-%d') BETWEEN CAST('$tglawal' AS DATE) AND CAST('$tglakhir' AS DATE)"; 
		$c3 = " AND DATE_FORMAT(tgl_transaksi, '%Y-%m-%d') < '$tglawal'"; 
	}else $c = $cm = $c1 = $c2 = $c3 = "";
	if($id_sumber!=""){
	...
	$result['q'] = "SELECT a.id_barang, jml_in, saldo, harga, stat, IF(b.id_barang IS NOT NULL, 1, 2) AS j, 
							tgl_transaksi, IFNULL(b.nama_barang, bk.nama_barang_kegiatan) AS nama_barang,
							id_transaksi_detail, IFNULL(b.keterangan, bk.keterangan) AS merk, tahun, 
							IFNULL(tgl_pemeriksaan, tgl_so) tgl_pemeriksaan, 
							IFNULL(no_ba_pemeriksaan, no_so) no_ba_pemeriksaan, tgl_pengadaan, no_kontrak
