<?php
	...
	$clause = "SELECT id_bar,id_gud,nama_sumber,id_sum,stat,SUM(hrgsattot_admin) AS hrgsat_admin,
	uuid_skpd FROM (
	SELECT id_bar,id_gud,nama_sumber,id_sum,stat,jml_admin,hrgsat_admin,(jml_admin*hrgsat_admin) AS hrgsattot_admin,uuid_skpd FROM (
	SELECT k.id_barang AS id_bar, k.id_gudang AS id_gud, d.nama_sumber AS nama_sumber, k.id_sumber_dana AS id_sum, 'b' AS stat,
	(k.jml_in-k.jml_out) AS jml_admin, harga AS hrgsat_admin, k.uuid_skpd FROM kartu_stok k 
	 LEFT JOIN ref_sumber_dana d ON d.id_sumber = k.id_sumber_dana
	WHERE k.soft_delete = 0 AND k.id_barang IS NOT NULL $a AND k.ta <= '$ta'
	GROUP BY k.id_barang,(k.jml_in-k.jml_out), k.harga,  k.id_sumber_dana , k.id_gudang) X ) z
	GROUP BY id_bar,id_gud,nama_sumber,id_sum,stat,uuid_skpd";
	...
	while($row = mysql_fetch_assoc($rs)){
		...
		$jmlad = mysql_fetch_assoc(mysql_query("SELECT (SUM(k1.jml_in)-SUM(k1.jml_out)) AS jml_admin, SUM(k1.jml_in) AS j1, SUM(k1.jml_out) AS j2 FROM kartu_stok k1
						...
						AND k1.id_sumber_dana = '$row[id_sum]'  "));
