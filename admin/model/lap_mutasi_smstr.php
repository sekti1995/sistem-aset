<?php
	...
	while($row = mysql_fetch_assoc($rs)){
		$awal = mysql_fetch_row(mysql_query("SELECT SUM(jml_in-jml_out) FROM kartu_stok 
											WHERE id_barang = '$row[id_barang]' AND DATE_FORMAT(tgl_transaksi, '%Y-%m-%d') < '$tglawal'
											AND soft_delete = 0 $idsub $idsum"));
		$ini = mysql_fetch_row(mysql_query("SELECT SUM(jml_in), SUM(jml_out) FROM kartu_stok WHERE id_barang = '$row[id_barang]' 
											AND DATE_FORMAT(tgl_transaksi, '%Y-%m-%d') BETWEEN '$tglawal' AND '$tglakhir'
											AND soft_delete = 0 AND kode<>'m' $idsub $idsum"));
		$harga = mysql_fetch_row(mysql_query("SELECT harga FROM kartu_stok 
											WHERE id_barang = '$row[id_barang]' AND DATE_FORMAT(tgl_transaksi, '%Y-%m-%d') <= '$tglakhir'
