<?php
	...
	if($akses == 2){
		...
		$koder1 = " AND k1.kode != 'r' AND k1.kode != 'os' AND ( k1.kode = 'a' OR k1.kode = 'i' OR k1.kode = 'ok' OR k1.kode = 's' OR k1.kode = 'd') ";
		$koder2 = " AND k2.kode != 'r' AND k2.kode != 'os' AND ( k2.kode = 'a' OR k2.kode = 'i' OR k2.kode = 'ok' OR k2.kode = 's' OR k2.kode = 'd') ";
		...
		$koder1 = " AND (k1.keterangan = 'r' OR k1.kode = 'a' OR k1.kode = 'i' OR k1.kode = 'ok' OR k1.kode = 's' OR k1.kode = 's' OR k1.kode = 'd') ";
		$koder2 = " AND (k2.keterangan = 'r' OR k2.kode = 'a' OR k2.kode = 'i' OR k2.kode = 'ok' OR k2.kode = 's' OR k2.kode = 's' OR k2.kode = 'd') ";
	...
	$clause = "SELECT a.id_transaksi_detail, a.id_barang, a.kode_trans, IFNULL(b.nama_barang, bk.nama_barang_kegiatan) AS nama_barang, 
					...
					IF(b.id_barang IS NOT NULL, 1, 2) AS j, IFNULL(s1.nama_satuan, s2.nama_satuan) AS satuan, IFNULL(s1.id_satuan, s2.id_satuan) AS id_satuan
				FROM (
				   SELECT k1.id_transaksi_detail, k1.id_barang, k1.kode AS kode_trans, 0 AS jml_in, k1.harga, (SUM(jml_in)-SUM(jml_out)) AS saldo, 0 AS jml_out
					...
					WHERE k1.soft_delete = 0 $koder1 $a1 $e1 $c1 $d1
				   ...
				   SELECT k2.id_transaksi_detail, k2.id_barang, k2.kode AS kode_trans, SUM(k2.jml_in), k2.harga, 0 AS saldo, SUM(k2.jml_out)
					...
					WHERE k2.soft_delete = 0  AND k2.kode <> 'm' $koder2 $a2 $e2 $c2 $d2
	...
	while($row = mysql_fetch_assoc($rs)){
		...
		// $row['nama_barang'] = $row['nama_barang']." ".$row["id_transaksi_detail"];
