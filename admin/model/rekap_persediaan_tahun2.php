<?php
		...
		$clause = "SELECT a.id_transaksi_detail, a.id_barang, a.kode_trans, IFNULL(b.nama_barang, bk.nama_barang_kegiatan) AS nama_barang, 
					...
					FROM (
					   SELECT k1.id_transaksi_detail, k1.id_barang, k1.kode AS kode_trans, 0 AS jml_in, k1.harga, (SUM(jml_in)-SUM(jml_out)) AS saldo, 0 AS jml_out
						FROM kartu_stok k1 LEFT JOIN ref_sub2_unit sub1 ON k1.uuid_skpd = sub1.uuid_sub2_unit WHERE k1.soft_delete = 0 $a1 $b1 $d1 $e1
					   ...
					   SELECT k2.id_transaksi_detail, k2.id_barang, k2.kode AS kode_trans, SUM(k2.jml_in), k2.harga, 0 AS saldo, SUM(k2.jml_out)
						FROM kartu_stok k2 LEFT JOIN ref_sub2_unit sub2 ON k2.uuid_skpd = sub2.uuid_sub2_unit WHERE k2.soft_delete = 0  AND k2.kode <> 'm' $a2 $b2 $d2 $e2
	...
	while($row = mysql_fetch_assoc($rs)){
		...
		//LALU
		$cek_keg_lalu = mysql_fetch_assoc(mysql_query("SELECT SUM(k1.jumlah*k1.harga) AS tot_harga FROM masuk_detail_rinci k1 LEFT JOIN masuk_detail k2 ON k1.id_masuk_detail = k2.id_masuk_detail WHERE k1.id_masuk_detail = '$row[id_transaksi_detail]' $ak1 "));
		//INI
		$cek_keg_ini = mysql_fetch_assoc(mysql_query("SELECT SUM(k1.jumlah*k1.harga) AS tot_harga FROM masuk_detail_rinci k1 LEFT JOIN masuk_detail k2 ON k1.id_masuk_detail = k2.id_masuk_detail WHERE k1.id_masuk_detail = '$row[id_transaksi_detail]' $ak2 "));
		...
		// $row['nama_barang'] = $row['nama_barang']." ".$row['id_barang'];
