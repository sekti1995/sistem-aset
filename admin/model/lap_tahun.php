<?php
	...
	$clause = "SELECT a.id_sumber_dana, a.id_barang, a.kode_trans, IFNULL(b.nama_barang, bk.nama_barang_kegiatan) AS nama_barang, 
					 IFNULL(b.id_jenis, bk.id_jenis) AS jenis, 
					 CONCAT_WS('.', j1.kd_kel, j1.kd_sub) AS kode_bar, j1.nama_jenis, b.kd_sub2,
				...
				FROM (
				   SELECT k1.id_sumber_dana, k1.id_barang, k1.kode AS kode_trans, 0 AS jml_in, k1.harga, (SUM(jml_in)-SUM(jml_out)) AS saldo, 0 AS jml_out
					FROM kartu_stok k1 WHERE k1.soft_delete = 0 $a1 $b1 $c1 $d1
					GROUP BY k1.id_barang, k1.harga HAVING saldo <> 0
				   UNION ALL
				   SELECT k2.id_sumber_dana, k2.id_barang, k2.kode AS kode_trans, SUM(k2.jml_in), k2.harga, 0 AS saldo, SUM(k2.jml_out)
				    FROM kartu_stok k2 WHERE k2.soft_delete = 0  AND k2.kode <> 'm' $a2 $b2 $c2 $d2
	...
	while($row = mysql_fetch_assoc($rs)){
		...
		// $row["nama_barang"] = $row["kode_bar"].".".$row["kd_sub2"]." ".$row["nama_barang"];
