<?php
	...
	if($bln!="" AND $bln!="00"){
		$c1 = " AND DATE_FORMAT(k1.tgl_transaksi, '%Y-%m') < '$ta-$bln'"; 
		$c2 = " AND DATE_FORMAT(k2.tgl_transaksi, '%m') = '$bln'";
		
		$c3 = " AND DATE_FORMAT(k1.tgl_masuk_rinci, '%Y-%m') < '$ta-$bln'"; 
		$c4 = " AND DATE_FORMAT(k1.tgl_masuk_rinci, '%m') = '$bln'";
	}else $c1 = $c2 = $c3 = $c4 = "";
	if($id_sum!=""){
	...
	$result = array();
	$clause = "SELECT b.kd_sub2, id_stok, a.id_transaksi_detail, a.id_barang, a.kode_trans, IFNULL(b.nama_barang, bk.nama_barang_kegiatan) AS nama_barang, 
				...
				FROM (
				   SELECT id_stok, k1.id_transaksi_detail, k1.id_barang, k1.kode AS kode_trans, 0 AS jml_in, k1.harga, (SUM(jml_in)-SUM(jml_out)) AS saldo, 0 AS jml_out
					FROM kartu_stok k1 
					LEFT JOIN ref_sub2_unit t4 ON k1.uuid_skpd = t4.uuid_sub2_unit
					WHERE k1.soft_delete = 0 $a1 $c1 $d1 $e1
					GROUP BY k1.id_barang, k1.harga HAVING saldo <> 0
				   UNION ALL
				   SELECT id_stok, k2.id_transaksi_detail, k2.id_barang, k2.kode AS kode_trans, SUM(k2.jml_in), k2.harga, 0 AS saldo, SUM(k2.jml_out)
				    FROM kartu_stok k2 
					LEFT JOIN ref_sub2_unit t4b ON k2.uuid_skpd = t4b.uuid_sub2_unit
					WHERE k2.soft_delete = 0  AND k2.kode <> 'm' $a2 $c2 $d2 $e2
	...
	while($row = mysql_fetch_assoc($rs)){
		...
		// $row['nama_barang'] = $row['nama_barang']." ".$row['id_stok'];
