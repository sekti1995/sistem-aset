<?php
	...
	if($idsub!="") $a = " AND (k.uuid_skpd = '$idsub' OR r.uuid_sub2_unit = '$idsub') ";
	else $a = " AND MD5(k.uuid_skpd) = '$_SESSION[uidunit]'";
	if($ta!="") $b = " AND k.ta = '$ta'";
	else $b = "";
	...
	$offset = ($page-1)*$rows;
	$result = array();
	$clause = "SELECT t.create_date, u.nm_sub2_unit AS nm_skpd, k.ta, no_ba_out AS nomor, DATE_FORMAT(tgl_ba_out, '%d-%m-%Y') AS tanggal, 
				...
				WHERE k.soft_delete=0 AND k.jenis_out = 's' $id_sub $where
				ORDER BY status, tgl_ba_out, create_date DESC";
	...
	while($row = mysql_fetch_assoc($rs)){
		...
		$j = mysql_fetch_assoc(mysql_query("SELECT SUM(jml_barang) AS jb FROM keluar_detail WHERE id_keluar = '$row[id]' AND soft_delete = 0 "));
		
		$row["tt"] = number_format($j["jb"], 2, ',', '.');
		
		
		
		$row["jam"] = substr($row["create_date"],-8);
		$row["create_date"] = balikTanggalIndo($row["create_date"]);
		$row["create_date"] = $row["create_date"]." ".$row["jam"];
